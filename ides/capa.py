...
import sys,os
from ides.models import Categorias,Funciona,Privacidad,UrlPendientes
...
def info_capas(u,p,i,w,d,l):
	...
	try:
		...
		pass
	try:
		result=urllib2.urlopen(request2,timeout=30)
	except urllib2.HTTPError,e:
		pass
	...
	if 'metadataLinks' in data['featureType']:
		...
		try:
			result2=urllib2.urlopen(request3,timeout=30)
		except:
			print(metadata)
...
def sube_capas(u,p,i,db):
	...
	for i in range(0,total_workspace):
			...
			print(ide+"/geoserver/rest/workspaces/"+workspace+".html")
			try:
				result4=urllib2.urlopen(request5,timeout=30)
			except:
				result4=[]
			for x in result4:
					if 'datastores' in x:
			...
			for datastore in datastores:
					web6 = ide+"/geoserver/rest/workspaces/"+workspace+"/datastores/"+datastore+".html"
					web7 = ide+"/geoserver/rest/workspaces/"+workspace+"/coveragestores/"+datastore+".html"
					...
					try:
						result5=urllib2.urlopen(request6,timeout=30)
						print web6
					except urllib2.HTTPError,e:
						pass
					try:
						result5=urllib2.urlopen(request7,timeout=30)
						print web7
					except urllib2.HTTPError,e:
						pass
					...
					for layer in layers:
							UrlPendientes(user=user,password=password,ide=ide,workspace=workspace,datastore=datastore,layer=layer).save()
